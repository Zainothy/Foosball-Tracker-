...
function exportPlayersCsv(state, seasonFilter = null) {
  const currentSeason =
    seasonFilter === null ? getCurrentSeason(state) : seasonFilter;
  const scopedGames = (state.games || []).filter((g) =>
    seasonFilter === "all" ? true : gameInSeason(g, currentSeason),
  );
  ...
  const rows = [
    ...
    ...(state.players || []).map((p) => {
      ...
      return [
        ...
        p.name,
        stats.pts || 0,
        p.mmr ?? CONFIG.STARTING_MMR,
        p.mmr_atk ?? p.mmr ?? CONFIG.STARTING_MMR,
        p.mmr_def ?? p.mmr ?? CONFIG.STARTING_MMR,
  ...
  const seasonLabel =
    seasonFilter === "all" ? "all-time" : currentSeason?.label || "current";
  downloadText(
    `foosball-players-${seasonLabel}-${new Date().toISOString().slice(0, 10)}.csv`,
    toCsv(rows),
    "text/csv",
  );
}
function exportGamesCsv(state, seasonFilter = null) {
  ...
  const prefRole = new Map(
    (state.players || []).map((p) => [p.id, p.preferredRole || "FLEX"]),
  );
  const rows = [
    [
      "game_id",
  ...
  for (const g of sorted) {
    ...
    for (const pid of allIds) {
      ...
      const aligned =
        role && pref !== "FLEX" ? (role === pref ? "1" : "0") : "";
      const teammates = ((side === "A" ? g.sideA : g.sideB) || [])
        .filter((id) => id !== pid)
        .map((id) => nameById.get(id) || id);
      const opps = ((side === "A" ? g.sideB : g.sideA) || []).map(
  ...
  const seasonLabel =
    seasonFilter === "all" ? "all-time" : currentSeason?.label || "current";
  downloadText(
    `foosball-games-${seasonLabel}-${new Date().toISOString().slice(0, 10)}.csv`,
    toCsv(rows),
    "text/csv",
  );
}
...
function computeWindowPlayerStats(players, games) {
  const stats = Object.fromEntries(
    (players || []).map((p) => [
      p.id,
      { wins: 0, losses: 0, pts: 0, streak: 0 },
    ]),
  );
  ...
  for (const g of sorted) {
    ...
    for (const id of winIds) {
      ...
      if (!s) continue;
      const gain =
        g.perPlayerGains?.[id] ?? g.playerDeltas?.[id]?.gain ?? g.ptsGain ?? 0;
      s.wins += 1;
      s.pts += gain;
    ...
    for (const id of losIds) {
      ...
      if (!s) continue;
      const loss =
        g.perPlayerLosses?.[id] ?? g.playerDeltas?.[id]?.loss ?? g.ptsLoss ?? 0;
      s.losses += 1;
...
function buildBracket(pool) {
  ...
  const hasAtkPref = (p) =>
    ["ATK", "attack"].includes(p.preferredRole || p.position);
  const hasDefPref = (p) =>
    ["DEF", "defense"].includes(p.preferredRole || p.position);
...
function resolveRunnerUp(bracket) {
  ...
  if (bracket.upper?.winner) {
    ...
    const loserGoals =
      loserSide === "A" ? bracket.upper.scoreA : bracket.upper.scoreB;
    candidates.push({
      ids: loserSide === "A" ? bracket.upper.sideA : bracket.upper.sideB,
      performance: calcPerf(
        loserGoals,
        (bracket.upper.scoreA || 0) + (bracket.upper.scoreB || 0),
        false,
      ),
    });
  }
  if (bracket.lower?.winner) {
    ...
    const loserGoals =
      loserSide === "A" ? bracket.lower.scoreA : bracket.lower.scoreB;
    candidates.push({
      ids: loserSide === "A" ? bracket.lower.sideA : bracket.lower.sideB,
      performance: calcPerf(
        loserGoals,
        (bracket.lower.scoreA || 0) + (bracket.lower.scoreB || 0),
        false,
      ),
    });
  }
  if (bracket.final?.winner) {
    ...
    const loserGoals =
      loserSide === "A" ? bracket.final.scoreA : bracket.final.scoreB;
    candidates.push({
      ids: loserSide === "A" ? bracket.final.sideA : bracket.final.sideB,
      performance: calcPerf(
        loserGoals,
        (bracket.final.scoreA || 0) + (bracket.final.scoreB || 0),
        true,
      ),
    });
  }
  ...
  return {
    runnerUp: candidates[0]?.ids ?? null,
    thirdPlace: candidates[1]?.ids ?? null,
  };
}
...
function getSeasonSummary(state, season) {
  ...
  const totalPts = seasonGames.reduce(
    (s, g) => s + (g.ptsGain || 0) + (g.ptsLoss || 0),
    0,
  );
  const sevenDaysAgo = new Date(Date.now() - 7 * MS_PER_DAY);
  const sevenDayGames = seasonGames.filter(
    (g) => new Date(g.date) >= sevenDaysAgo,
  );
  const sevenDayStats = computeWindowPlayerStats(state.players, sevenDayGames);
  const topClimber = [...state.players].sort(
    (a, b) => (sevenDayStats[b.id]?.pts || 0) - (sevenDayStats[a.id]?.pts || 0),
  )[0];
  const topClimberPts = topClimber ? sevenDayStats[topClimber.id]?.pts || 0 : 0;
  const mostActive = [...state.players].sort(
    (a, b) =>
      sevenDayGames.filter(
        (g) => g.sideA.includes(b.id) || g.sideB.includes(b.id),
      ).length -
      sevenDayGames.filter(
        (g) => g.sideA.includes(a.id) || g.sideB.includes(a.id),
      ).length,
  )[0];
  const activeCount = mostActive
    ? sevenDayGames.filter(
        (g) =>
          g.sideA.includes(mostActive.id) || g.sideB.includes(mostActive.id),
      ).length
    : 0;
  return {
    matchCount,
...
function getBestTeammate(playerId, games) {
  ...
  const best = Object.entries(teammates).sort(
    (a, b) =>
...
function getToughestOpponent(playerId, games) {
  ...
  const toughest = Object.entries(opponents).sort(
    (a, b) =>
...
function getAvgGoals(playerId, games) {
  ...
  games.forEach((g) => {
    const onA = g.sideA.includes(playerId);
    const [scoreFor, scoreAgainst] = onA
      ? [g.scoreA, g.scoreB]
      : [g.scoreB, g.scoreA];
    goalsFor += scoreFor;
...
function cycleRoleForSide(rows, rowId, sideIds) {
  return rows.map((row) => {
    ...
    return {
      ...row,
      roles: { ...row.roles, [currentAtk]: "DEF", [other]: "ATK" },
    };
  });
}
...
function normaliseState(s) {
  ...
  return {
    players: (s.players || []).map((p) => ({
      ...
      wins_atk: p.wins_atk ?? 0,
      losses_atk: p.losses_atk ?? 0,
      wins_def: p.wins_def ?? 0,
      losses_def: p.losses_def ?? 0,
      preferredRole:
        p.preferredRole ??
        (p.position === "attack"
          ? "ATK"
        ...
        ? [
            {
              id: "season_1",
              label: "Season 1",
              startAt: s.seasonStart,
              endAt: null,
              createdAt: s.seasonStart,
            },
          ]
        : []),
    _meta: s._meta || {},
...
function checkSuspiciousGame(game) {
  ...
  if (total > 50)
    warnings.push({
      type: "totalScore",
      value: total,
      msg: "Total score unusually high (>50)",
    });
  if (margin > 15)
    warnings.push({
      type: "margin",
      value: margin,
      msg: "Score margin unusually large (>15)",
    });
  if (total === 0)
    warnings.push({ type: "zeroScore", msg: "Both teams scored 0" });
  if (game.scoreA === game.scoreB)
    warnings.push({ type: "draw", msg: "Draw - game should have a winner" });
  return warnings;
}
...
async function cleanupBackupsIfNeeded() {
  try {
    ...
    await supabase
      ...
      .lt(
        "saved_at",
        new Date(now - BACKUP_RETENTION_DAYS * 86400000).toISOString(),
      );
...
async function maybeBackupState(stateToBackup) {
  try {
    const hasData =
      (stateToBackup?.players?.length || 0) > 0 ||
      (stateToBackup?.games?.length || 0) > 0;
    if (!hasData) return;
    const now = Date.now();
...
async function _flushSave() {
  ...
  async function handleConflict() {
    ...
    try {
      ...
      console.error("[sync] conflict fetch failed:", e);
    }
  }
  try {
    const { data: rpcData, error: rpcErr } = await supabase.rpc(
      "update_state_versioned",
      { expected_v: baseV, new_state: slimmed },
    );
    ...
    if (_sq.retries < MAX) {
      ...
      console.warn(
        "[sync] retry " + _sq.retries + "/" + MAX + " in " + delay + "ms:",
        err.message,
      );
...
function StreakBadge({ streak, streakPower = 0, showMult = false }) {
  ...
  return s > 0 ? (
    <span className="text-g bold">
      ▲{s}
      {showMult && (
        <span className="xs" style={{ opacity: 0.7 }}>
          {" "}
          ×{m.toFixed(2)}
        </span>
      )}
...
function Pips({ used }) {
  return (
    <>
      {Array.from({ length: CONFIG.MAX_PLACEMENTS_PER_MONTH }).map((_, i) => (
...
function PosBadge({ pos }) {
  if (!pos || pos === "none" || (Array.isArray(pos) && pos.length === 0))
    return <span className="text-dd xs">—</span>;
  const positions = Array.isArray(pos) ? pos : [pos];
  const badges = positions
    .map((p) => {
      if (p === "attack")
        return (
    ...
    .filter(Boolean);
  return (
    <div className="fac" style={{ gap: 3, flexWrap: "wrap" }}>
      {badges}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      >
        ...
        <span style={{ marginLeft: 4, color: "var(--dimmer)", fontSize: 10 }}>
          placed
        </span>
      </div>
      <div
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <path
          d="M3.5 10 L2 15 L6.5 12.5 L11 15 L9.5 10"
          fill="#b0c8c0"
          opacity="0.6"
        />
      </svg>
...
function lastNResults(pid, games, n = 5) {
  return [...games]
    ...
    .map((g) => {
      ...
      return (onA && g.winner === "A") || (!onA && g.winner === "B")
        ? "W"
        : "L";
    });
}
    ...
    <Modal onClose={onCancel}>
      <div className="confirm-modal">
        <div
          ...
          style={{ color: danger ? "var(--red)" : "var(--amber)" }}
        >
          {title}
        </div>
...
function PlayerProfile({
  ...
  const selectedSeason =
    seasonMode === "all"
      ? null
      : (state.seasons || []).find((s) => s.id === selectedSeasonId) ||
        currentSeason ||
        null;
  const myGames = [...state.games]
    .filter(
      (g) =>
        (g.sideA.includes(player.id) || g.sideB.includes(player.id)) &&
        gameInSeason(g, selectedSeason),
    )
    .sort((a, b) => new Date(b.date) - new Date(a.date));
  const rankBase = [...state.players].sort(
    (a, b) => (b.pts || 0) - (a.pts || 0),
  );
  const rank = rankBase.findIndex((p) => p.id === player.id) + 1;
  const seasonWindowStats = computeWindowPlayerStats(
    state.players,
    state.games.filter((g) => gameInSeason(g, selectedSeason)),
  );
  const scopedStats = seasonWindowStats[player.id] || {
    wins: 0,
    losses: 0,
    pts: 0,
    streak: 0,
  ...
  const displayLosses =
    seasonMode === "all" ? player.losses : scopedStats.losses;
  const displayPts = seasonMode === "all" ? player.pts || 0 : scopedStats.pts;
  const displayStreak =
    seasonMode === "all" ? player.streak : scopedStats.streak;
...
function EditPlayerModal({ player, state, setState, showToast, onClose }) {
  ...
  function save() {
    const newPts = parseInt(pts),
      newStreak = parseInt(streak);
    if (isNaN(newPts) || isNaN(newStreak)) {
    ...
    setState((s) => ({
      ...
      players: s.players.map((p) =>
        p.id === player.id
          ? {
              ...p,
              championships: (p.championships || []).filter(
                (_, idx) => idx !== i,
              ),
            }
          : p,
  ...
  function recalcPlayer() {
    setConfirm({
      ...
      onConfirm: () => {
        ...
        showToast("All stats recalculated");
        setConfirm(null);
        onClose();
      },
    });
    ...
    <>
      <Modal onClose={onClose}>
        ...
        <div className="grid-2">
          ...
          <div className="field">
            <label className="lbl">Streak (+win / -loss)</label>
            <input
              className="inp inp-edit"
              type="number"
              value={streak}
        ...
        <div className="msg msg-w sm">
          Manually editing pts/streak will diverge from game history. Use
          recalculate to re-sync.
        </div>
        ...
        <div className="grid-2 mt8">
          ...
          <div className="field">
            <label className="lbl">Partner (optional)</label>
            <input
              className="inp"
              placeholder="Teammate name"
              value={champPartner}
              onChange={(e) => setChampPartner(e.target.value)}
            />
          </div>
        </div>
        <button className="btn btn-warn btn-sm" onClick={addChamp}>
          + Add Championship
        </button>
        <div className="divider" />
        <div
        ...
        >
          <button className="btn btn-g btn-sm" onClick={recalcPlayer}>
            Recalculate All from Games
          </button>
          <div className="fac">
...
function GameDetail({
  ...
  function savePenalties() {
    ...
    const editedGames = state.games.map((g) =>
    ...
    const { players: newPlayers, games: newGames } = replayGames(
      basePlayers,
      editedGames,
      state.seasonStart,
      state.seasons,
    ...
    const mergedPlayers = newPlayers.map((p) => {
      ...
      return {
        ...p,
        name: orig?.name || p.name,
        championships: orig?.championships || [],
        runnerUps: orig?.runnerUps || [],
        thirdPlaces: orig?.thirdPlaces || [],
        position: orig?.position || p.position,
      };
    });
    ...
    const totalCards = Object.values(penalties).reduce(
      (s, v) => (v.yellow || 0) + (v.red || 0) + s,
      0,
    );
    showToast(
      totalCards > 0
        ? "Penalties applied & stats updated"
        : "Penalties cleared",
    );
    setEditing(false);
    onClose();
  ...
  function penaltyTotal(pid) {
    ...
    return (
      (p.yellow || 0) * CONFIG.YELLOW_CARD_PTS +
      (p.red || 0) * CONFIG.RED_CARD_PTS
    );
  }

  const hasPenalties = Object.values(game.penalties || {}).some(
      ...
      <Modal onClose={onClose} variant="match-detail" label="Match detail">
            ...
            {sA.map((p) => {
              ...
              return (
                <div key={p.id} style={{ marginBottom: 4 }}>
                  ...
                  >
                    <span
                      className={`bold ${game.winner === "A" ? "text-g" : "text-r"}`}
                      style={{ fontSize: 14 }}
                    >
                      {p.name}
                    </span>
                        ...
                        {["ATK", "DEF", "FLEX"].map((r) => (
                          <button
                            ...
                            style={{
                              ...
                              fontWeight: editRoles[p.id] === r ? 700 : 400,
                            }}
                            onClick={() =>
                              setEditRoles((prev) => ({
                                ...prev,
                                [p.id]: prev[p.id] === r ? null : r,
                              }))
                            }
                          >
                            {r === "ATK"
                  ...
                  <div className="xs text-dd">
                    {game.winner === "A" ? (
                      <span className="text-g">+{gain}pts</span>
                    ) : (
                      <span className="text-r">−{loss}pts</span>
                    )}
                    {pen > 0 && (
            ...
            {editing ? (
              ...
              >
                <div className="fac" style={{ gap: 6 }}>
                  ...
                  />
                  <span className="text-dd" style={{ fontSize: 18 }}>
                    –
                  </span>
                  <input
                    className="inp inp-edit"
            ...
            {sB.map((p) => {
              ...
              return (
                <div key={p.id} style={{ marginBottom: 4 }}>
                  ...
                  >
                    <span
                      className={`bold ${game.winner === "B" ? "text-g" : "text-r"}`}
                      style={{ fontSize: 14 }}
                    >
                      {p.name}
                    </span>
        ...
        {!editing &&
          (() => {
            ...
            const avgWinRank = placedWinners.length
              ? placedWinners.reduce((s, id) => s + rankOf(id), 0) /
                placedWinners.length
              : null;
            const avgLosRank = placedLosers.length
              ? placedLosers.reduce((s, id) => s + rankOf(id), 0) /
                placedLosers.length
              : null;
            const rankImbalance =
              avgWinRank !== null && avgLosRank !== null
                ? Math.abs(avgWinRank - avgLosRank)
                : 0;
            const winnerOutrankedLosers =
              avgWinRank !== null &&
              avgLosRank !== null &&
              avgWinRank < avgLosRank;
            const canShowRankBanner =
              avgWinRank !== null && avgLosRank !== null;
              ...
              >
                ...
                >
                  ...
                  <span className="xs text-dd">
                    {canShowRankBanner
                      ? `Rank gap: ${rankImbalance.toFixed(1)}`
                      : "Placement game"}
                  </span>
                </div>
                <div
                ...
                >
                  {allPlayers.map((p) => {
                    ...
                    if (!f) return null;
                    const isWinner = (
                      game.winner === "A" ? game.sideA : game.sideB
                    ).includes(p.id);
                    const pts = isWinner
                  ...
                  <div
                    ...
                    style={{ lineHeight: 1.6, paddingTop: 2 }}
                  >
                    Match quality = 70% MMR gap + 30% rank gap. FLEX = mid-game
                    swap, neutral on all tracks.
                  </div>
                </div>
              </div>
            );
        ...
        {isAdmin && (
          <details className="discipline-panel">
                  ...
                  >
                    ...
                    <div className="fac" style={{ gap: 4 }}>
                      ...
                      <button
                        ...
                        style={{ padding: "2px 7px", minWidth: 22 }}
                        onClick={() =>
                          setPenalty(p.id, "yellow", (pen.yellow || 0) + 1)
                        }
                      >
                        +
                      </button>
                      <span className="xs text-dd">
                        −{CONFIG.YELLOW_CARD_PTS}pts ea
                      </span>
                    </div>
                    <div className="fac" style={{ gap: 4 }}>
                      <span style={{ fontSize: 16 }}>
                      ...
                      <button
                        ...
                        style={{ padding: "2px 7px", minWidth: 22 }}
                        onClick={() =>
                          setPenalty(p.id, "red", (pen.red || 0) + 1)
                        }
                      >
                        +
                      </button>
                      <span className="xs text-dd">
                        −{CONFIG.RED_CARD_PTS}pts ea
                      </span>
                    </div>
                    {penaltyTotal(p.id) > 0 && (
                      <span
            ...
            {!editing && (
              <button
                className="btn btn-warn w-full mt8"
        ...
        >
          {editing ? (
            ...
            <button className="btn btn-g w-full" onClick={onClose}>
              Close
            </button>
          )}
        </div>
...
function LiveTicker({ games, players, finals, monthKey, onNavToPlay }) {
  ...
  const liveMatchKey =
    bracket &&
    ["upper", "lower", "final"].find(
      (k) => liveScores[k]?.active && bracket[k] && !bracket[k].winner,
    );
  if (liveMatchKey) {
    ...
    const lA = liveScores[liveMatchKey]?.scoreA ?? 0,
      lB = liveScores[liveMatchKey]?.scoreB ?? 0;
    ...
    return (
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse 80% 300% at 0% 50%,rgba(232,184,74,.14),var(--s1))",
      ...
      >
        ...
        <span style={{ flex: 1 }}>
          <span
            style={{
              fontWeight: 700,
              color: leading === "A" ? "var(--green)" : "var(--text)",
            }}
          >
            {pA}
          </span>
          <span
            className="disp text-am"
            style={{ margin: "0 10px", fontSize: 18, fontWeight: 700 }}
          >
            {lA}–{lB}
          </span>
          <span
            style={{
              fontWeight: 700,
              color: leading === "B" ? "var(--green)" : "var(--text)",
            }}
          >
            {pB}
          </span>
          <span className="text-dd xs" style={{ marginLeft: 8 }}>
            {labMap[liveMatchKey]}
          </span>
        </span>
  ...
  return (
    <div
      style={{
        background:
          "radial-gradient(ellipse 80% 300% at 0% 50%,rgba(94,201,138,.12),var(--s1))",
    ...
    >
      <span className="tag tag-w" style={{ flexShrink: 0 }}>
        RESULT
      </span>
      <span style={{ flex: 1 }}>
        <span className="text-g bold">
          {wIds.map((id) => pName(id, players)).join(" & ")}
        </span>
        <span className="text-dd"> beat </span>
        <span>{lIds.map((id) => pName(id, players)).join(" & ")}</span>
        <span
          className="text-am bold"
          style={{ marginLeft: 8, fontFamily: "var(--disp)" }}
        >
      ...
      <button
        onClick={() => {
          _dismissedTickers.add(tickerId);
          forceUpdate((n) => n + 1);
        }}
        style={{
          background: "none",
  ...
  return (
    <>
      <div className="stack page-fade">
        ...
        {(() => {
          const placedRanked = ranked
            .filter(
              (p) =>
                (state.monthlyPlacements[placementKey] || {})[p.id] >=
                CONFIG.MAX_PLACEMENTS_PER_MONTH,
            )
            .slice(0, 4);
          return (
            placedRanked.length >= 2 && (
              <div
                ...
                onClick={() => onNavToPlay()}
                onMouseEnter={(e) =>
                  (e.currentTarget.style.borderColor = "var(--amber-d)")
                }
                  ...
                  {placedRanked.map((p, i) => (
                    <div
                      ...
                      style={{
                        ...
                        background:
                          i === 0
                            ? "radial-gradient(ellipse 120% 120% at 100% 100%,rgba(232,184,74,.15),var(--s2))"
                            : i === 1
                              ? "radial-gradient(ellipse 120% 120% at 100% 100%,rgba(192,200,196,.08),var(--s2))"
                              : i === 2
                                ? "radial-gradient(ellipse 120% 120% at 100% 100%,rgba(200,134,74,.08),var(--s2))"
                                : "var(--s2)",
      ...
      {showRecalcConfirm && (
        <ConfirmDialog
          ...
          msg="This will replay every game in history and rewrite all player points, MMR, streaks, wins, losses, and the pts shown in match history."
          onConfirm={doRecalc}
...
function OnboardView({ state, setState, showToast, onEdit }) {
  ...
  function addSingle() {
    ...
    setState((s) =>
      logAdmin({ ...s, players: [...s.players, newPlayer] }, "ADD_PLAYER", {
        name,
      }),
    );
    setSingle("");
...
function LogView({ state, setState, showToast, active, syncStatus }) {
  ...
  function setRowPenalty(rowId, pid, type, delta) {
    setRows((r) =>
      r.map((row) => {
        ...
        return {
          ...row,
          penalties: { ...row.penalties, [pid]: { ...cur, [type]: newVal } },
        };
      }),
    );
  }
    ...
    setRows((r) =>
      r.map((row) => {
        ...
        if (pref === "ATK" || pref === "DEF") {
          ...
          const alreadyHasRole = sideIds
            .filter((id) => id !== pid)
            .some((id) => newRoles[id] === pref);
  ...
  function saveTpl() {
    ...
    const t = {
      name: tplName,
      rows: rows.map((r) => ({ sideA: r.sideA, sideB: r.sideB })),
    };
  ...
  function loadTpl(t) {
    setRows(
      t.rows.map((r) => ({ ...EMPTY_ROW(), sideA: r.sideA, sideB: r.sideB })),
    );
  ...
  function submitAll(skipDuplicateCheck = false) {
    ...
    for (const row of rows) {
      if (row.sideA.length !== 2 || row.sideB.length !== 2) {
        newErrors[row.id] = "Each side needs exactly 2 players";
        continue;
      }
      if (new Set([...row.sideA, ...row.sideB]).size < 4) {
        newErrors[row.id] = "A player appears on both sides";
        continue;
      }
      const sA = parseInt(row.scoreA, 10),
        sB = parseInt(row.scoreB, 10);
      if (isNaN(sA) || isNaN(sB) || sA < 0 || sB < 0) {
        newErrors[row.id] = "Invalid scores";
        continue;
      }
      if (sA === sB) {
        newErrors[row.id] = "No draws allowed";
        continue;
      }
      ...
      if (assignedRoles.length > 0 && assignedRoles.length < 4) {
        newErrors[row.id] =
          "Assign roles to all 4 players, or leave all blank. Use FLEX for mid-game swaps.";
        continue;
      }
      // Validate ATK/DEF balance per side (FLEX bypasses this)
      if (assignedRoles.length === 4) {
        const sideAHasFlex = row.sideA.some(
          (pid) => row.roles?.[pid] === "FLEX",
        );
        const sideBHasFlex = row.sideB.some(
          (pid) => row.roles?.[pid] === "FLEX",
        );
        if (!sideAHasFlex) {
          const atkA = row.sideA.filter(
            (pid) => row.roles?.[pid] === "ATK",
          ).length;
          if (atkA !== 1) {
            newErrors[row.id] =
              "Side A needs exactly 1 ATK and 1 DEF (or use FLEX for a swap)";
            continue;
          }
        }
        if (!sideBHasFlex) {
          const atkB = row.sideB.filter(
            (pid) => row.roles?.[pid] === "ATK",
          ).length;
          if (atkB !== 1) {
            newErrors[row.id] =
              "Side B needs exactly 1 ATK and 1 DEF (or use FLEX for a swap)";
            continue;
          }
        }
      }
    ...
    for (const row of rows) {
      ...
      if (Math.abs(sA - sB) > 15)
        issues.push(`Margin ${Math.abs(sA - sB)} (very lopsided)`);
    ...
    const snapshot = {
      players: state.players,
      games: state.games,
      monthlyPlacements: state.monthlyPlacements,
    };
    ...
    const pendingGames = rows.map((row) => {
      ...
      const gamePenalties =
        Object.keys(row.penalties || {}).length > 0 ? row.penalties : undefined;
      const allPids = [...row.sideA, ...row.sideB];
      const assignedRoles = allPids.filter((pid) => row.roles?.[pid]);
      return {
        id: crypto.randomUUID(),
  ...
  function undoLast() {
    ...
    setState((s) => ({
      ...
      games: prev.games,
      monthlyPlacements: prev.monthlyPlacements,
    }));
    setUndoStack(rest);
      ...
      <div className="stack page-fade">
              ...
              {lastLogged.games.map((g) => {
                ...
                return (
                  ...
                  >
                    <span className="text-g bold">
                      {wIds
                        .map((id) => pName(id, lastLogged.players))
                        .join(" & ")}
                    </span>
                    <span className="disp text-am" style={{ fontSize: 18 }}>
                      {g.scoreA}–{g.scoreB}
                    </span>
                    ...
                    <span className="xs text-dd" style={{ marginLeft: "auto" }}>
                      {wIds.map((id) => (
                        <span
                          ...
                          style={{ marginRight: 6 }}
                        >
                          +{g.perPlayerGains?.[id] ?? g.ptsGain}{" "}
                          {pName(id, lastLogged.players).split(" ")[0]}
                        </span>
                      ))}
                      {lIds.map((id) => (
                        <span
                          ...
                          style={{ marginRight: 6 }}
                        >
                          −{g.perPlayerLosses?.[id] ?? g.ptsLoss}{" "}
                          {pName(id, lastLogged.players).split(" ")[0]}
                        </span>
                      ))}
                    </span>
        ...
        {templates.length > 0 && (
          <div className="card">
            ...
            <div
              style={{ padding: 14, display: "flex", gap: 8, flexWrap: "wrap" }}
            >
              {templates.map((t, i) => (
        ...
        <div className="card">
          ...
          <div style={{ padding: 14 }}>
            {rows.map((row, ri) => {
              ...
              let prev = null;
              if (canPreview) {
                const wIds = sA > sB ? row.sideA : row.sideB,
                  lIds = sA > sB ? row.sideB : row.sideA;
                const currentRanked = [...state.players].sort(
                  (a, b) => (b.pts || 0) - (a.pts || 0),
                );
                const rankOf = (id) => {
                  const i = currentRanked.findIndex((p) => p.id === id);
                  return i === -1 ? currentRanked.length : i;
                };
                const monthPlacements =
                  state.monthlyPlacements?.[getCurrentPlacementKey(state)] ||
                  {};
                const isPlaced = (pid) =>
                  (monthPlacements[pid] || 0) >=
                  CONFIG.MAX_PLACEMENTS_PER_MONTH;
                const oppWinMMR = avg(wIds, state.players, "mmr"),
                  oppLosMMR = avg(lIds, state.players, "mmr");
                const oppAvgRankPlaced = (ids) => {
                  const placed = ids.filter(isPlaced);
                  if (!placed.length) return null;
                  return (
                    placed.reduce((s, id) => s + rankOf(id), 0) / placed.length
                  );
                };
              ...
              return (
                ...
                >
                  <div className="fbc mb8">
                    ...
                    >
                      ...
                      {(() => {
                        ...
                        const sAv = parseInt(row.scoreA, 10),
                          sBv = parseInt(row.scoreB, 10);
                        const scoresOk =
                          !isNaN(sAv) &&
                          !isNaN(sBv) &&
                          sAv >= 0 &&
                        ...
                        if (!playersOk)
                          return (
                            ...
                            >
                              ● {4 - row.sideA.length - row.sideB.length} player
                              {4 - row.sideA.length - row.sideB.length !== 1
                                ? "s"
                                : ""}{" "}
                              needed
                            </span>
                        ...
                        {[...state.players]
                          ...
                          .filter(
                            (p) =>
                              ...
                              p.name
                                .toLowerCase()
                                .includes(row.searchA.toLowerCase()),
                          )
                          .map((p) => {
                            const onA = row.sideA.includes(p.id),
                              onB = row.sideB.includes(p.id),
                    ...
                    >
                      ...
                      {prev && (
                        ...
                        >
                          ...
                          {prev.wIds.some(
                            (id) =>
                              (prev.perPlayer[id]?.qualityScore ?? 1) < 0.85,
                          ) && (
                            <div
                              style={{
                                color: "var(--orange)",
                        ...
                        {[...state.players]
                          ...
                          .filter(
                            (p) =>
                              ...
                              p.name
                                .toLowerCase()
                                .includes(row.searchB.toLowerCase()),
                          )
                          .map((p) => {
                            const onA = row.sideA.includes(p.id),
                              onB = row.sideB.includes(p.id),
                  ...
                  {row.sideA.length + row.sideB.length > 0 &&
                    (() => {
                      const allPids = [...row.sideA, ...row.sideB];
                      const assignedCount = allPids.filter(
                        (id) => row.roles?.[id],
                      ).length;
                      const fullyAssigned =
                        assignedCount === 4 && allPids.length === 4;
                      const partiallyAssigned =
                        assignedCount > 0 && !fullyAssigned;
                      const bothSidesFull =
                        row.sideA.length === 2 && row.sideB.length === 2;
                      ...
                      const RoleSlot = ({
                        ...
                        const occupant = sideIds.find(
                          (id) => row.roles?.[id] === roleToShow,
                        );
                        const isFlex = sideIds.every(
                          (id) => row.roles?.[id] === "FLEX",
                        );
                        if (isFlex)
                          return (
                            <div
                        ...
                        return (
                          <div
                            ...
                            title={
                              occupant
                                ? `${pName(occupant, state.players)} — click to swap`
                                : "Click to assign"
                            }
                          >
                            <div
                        ...
                        >
                          ...
                          >
                            ...
                            <div className="fac" style={{ gap: 6 }}>
                              {fullyAssigned ? (
                                ...
                                >
                                  Assign all 4 to enable ATK/DEF MMR
                                </span>
                              ) : (
                                <span className="xs text-dd">
                              ...
                              {assignedCount > 0 && (
                                <button
                                  ...
                                  onClick={() =>
                                    setRows((r) =>
                                      clearRolesForSide(
                                        clearRolesForSide(r, row.id, row.sideA),
                                        row.id,
                                        row.sideB,
                                      ),
                                    )
                              ...
                              <div>
                                ...
                                >
                                  <RoleSlot
                                    ...
                                    onClickSlot={() =>
                                      setRows((r) =>
                                        cycleRoleForSide(r, row.id, row.sideA),
                                      )
                                    }
                                  />
                                  <RoleSlot
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    setRows((r) =>
                                      setFlexForSide(r, row.id, row.sideA),
                                    )
                                  }
                              ...
                              <div>
                                ...
                                >
                                  <RoleSlot
                                    ...
                                    onClickSlot={() =>
                                      setRows((r) =>
                                        cycleRoleForSide(r, row.id, row.sideB),
                                      )
                                    }
                                  />
                                  <RoleSlot
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    setRows((r) =>
                                      setFlexForSide(r, row.id, row.sideB),
                                    )
                                  }
                                  title="Team swapped positions at the 5-goal mark (§3.7)"
                                >
                          ...
                          {partiallyAssigned && !bothSidesFull && (
                            <div
                              ...
                              style={{ fontStyle: "italic" }}
                            >
                              Finish adding players to assign roles
                            </div>
                          )}
                        </div>
                      );
            ...
            >
              ...
              <input
                ...
                style={{ width: 150 }}
              />
              <button className="btn btn-g" onClick={saveTpl}>
                Save Template
              </button>
              {undoStack.length > 0 && (
...
function FinalsDateEditor({ finalsDate, setState, showToast, isAdmin }) {
  ...
  const [hh, setHh] = useState(
    parsed ? String(parsed.getHours()).padStart(2, "0") : "18",
  );
...
function FinalsView({ state, setState, isAdmin, showToast }) {
  ...
  function getCountdown() {
    ...
    if (diff <= 0)
      return { days: "00", hours: "00", mins: "00", secs: "00", diff: 0 };
    return {
  ...
  function pickPlayer(pid) {
    ...
    setSlots((prev) => ({
      ...prev,
      [pickingTeam]: [...prev[pickingTeam], pid],
    }));
  }
  function removeFromSlot(team, pid) {
    const clears = {
      upperA: { ...EMPTY_SLOTS, upperA: slots.upperA.filter((x) => x !== pid) },
      upperB: {
        ...slots,
        upperB: slots.upperB.filter((x) => x !== pid),
        lowerA: [],
        lowerB: [],
      },
      lowerA: {
        ...slots,
        lowerA: slots.lowerA.filter((x) => x !== pid),
        lowerB: [],
      },
  ...
  function confirmManual() {
    if (slots.upperA.length !== 2 || slots.upperB.length !== 2) {
      showToast("Semi 1 needs 2 players per side", "error");
      return;
    }
    if (wantLower && (slots.lowerA.length !== 2 || slots.lowerB.length !== 2)) {
      showToast("Semi 2 needs 2 players per side", "error");
      return;
    }
    const hasLower =
      wantLower && slots.lowerA.length === 2 && slots.lowerB.length === 2;
    const bracket = {
      upper: {
        sideA: slots.upperA,
        sideB: slots.upperB,
    ...
    setState((s) => {
      ...
      if (match === "upper" || match === "lower") {
        ...
        if (semisComplete) {
          b.final = {
            sideA: upperWinner,
            sideB: hasLower ? lowerWinner : null,
            winner: null,
            scoreA: null,
            scoreB: null,
          };
  ...
  function awardChampionship() {
    ...
    const champWonSemi =
      (bracket.upper?.winner &&
        (bracket.upper.winner === "A"
          ? bracket.upper.sideA
          : bracket.upper.sideB
        ).every((id) => champIds.includes(id))) ||
      (bracket.lower?.winner &&
        (bracket.lower.winner === "A"
          ? bracket.lower.sideA
          : bracket.lower.sideB
        ).every((id) => champIds.includes(id)));
    ...
    setState((s) => ({
      ...
      players: s.players.map((p) => {
        ...
        if (isChamp) {
          ...
          const already = (p.championships || []).some(
            (c) => c.month === monthKey,
          );
          if (already) return p;
          return {
            ...
            championships: [
              ...(p.championships || []),
              { month: monthKey, partner, undefeated: isUndefeated },
            ],
          };
        }
        if (isRunnerUp) {
          ...
          return {
            ...p,
            runnerUps: [...(p.runnerUps || []), { month: monthKey, partner }],
          };
        }
        // Third place
        ...
        return {
          ...p,
          thirdPlaces: [...(p.thirdPlaces || []), { month: monthKey, partner }],
        };
      }),
    }));
  ...
  function getLive(matchKey) {
    return (
      state.finals?.[monthKey]?.liveScores?.[matchKey] || {
        scoreA: 0,
        scoreB: 0,
        active: false,
      }
        ...
        >
          <div
            style={{
              ...
              background:
                m.winner === "A"
                  ? "rgba(94,201,138,.08)"
                  : isLive && live.scoreA > live.scoreB
                    ? "rgba(94,201,138,.04)"
                    : "transparent",
              display: "flex",
              justifyContent: "space-between",
          ...
          >
                ...
                <div key={i} className="fac" style={{ gap: 6 }}>
                  <span
                    style={{
                      fontWeight: 600,
                      fontSize: 13,
                      color: m.winner === "A" ? "var(--green)" : "var(--text)",
                    }}
                  >
                    {pl.name}
                  </span>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              {done && (
                <span
                  className="disp text-am"
                  style={{ fontSize: 26, marginLeft: 12 }}
                >
                  {m.scoreA}
                </span>
              )}
              {m.winner === "A" && (
                <span className="tag tag-w" style={{ marginLeft: 8 }}>
                  WIN
                </span>
              )}
            </div>
          ...
          <div
            style={{
              ...
              background:
                m.winner === "B"
                  ? "rgba(94,201,138,.08)"
                  : isLive && live.scoreB > live.scoreA
                    ? "rgba(94,201,138,.04)"
                    : "transparent",
              display: "flex",
              justifyContent: "space-between",
          ...
          >
            {sideBReady ? (
              ...
              <span className="text-dd xs" style={{ fontStyle: "italic" }}>
                Awaiting other semi…
              </span>
            )}
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
  ...
  const placedRanked = ranked.filter(
    (p) =>
      (state.monthlyPlacements?.[getCurrentPlacementKey(state)] || {})[p.id] >=
      CONFIG.MAX_PLACEMENTS_PER_MONTH,
  );
  ...
  function initFinals() {
    if (placedRanked.length < 4) {
      showToast("Need at least 4 placed players", "error");
      return;
    }
  ...
  if (!finals) {
    const slotLabel = (key) =>
      ({
        upperA: "Semi 1 · Team A",
        upperB: "Semi 1 · Team B",
        lowerA: "Semi 2 · Team A",
        lowerB: "Semi 2 · Team B",
      })[key] || key;
    const allSlotKeys = wantLower
    ...
    const filteredUnassigned = !bracketSearch
      ? unassigned
      : unassigned.filter((p) =>
          p.name.toLowerCase().includes(bracketSearch.toLowerCase()),
        );
          ...
          {manualMode && isAdmin && (
            ...
            >
              ...
              <div
                style={{
                  display: "grid",
                  gridTemplateColumns: wantLower ? "1fr 24px 1fr" : "1fr",
                  gap: 10,
                  marginBottom: 14,
                }}
              >
                ...
                <div>
                  ...
                  {["upperA", "upperB"].map((slotKey, idx) => {
                    ...
                    return (
                      ...
                      >
                        {isActive && (
                          <div className="picking-label">Picking now</div>
                        )}
                        <div
                          className="xs"
                          style={{
                        ...
                        {slots[slotKey].length === 0 ? (
                          <div
                            ...
                            style={{ fontStyle: "italic" }}
                          >
                            {isActive ? "← select from list below" : "empty"}
                          </div>
                        ) : (
                          <div
                            style={{
                          ...
                          >
                            {slots[slotKey].map((id) => (
                              <span
                                ...
                                onClick={() => removeFromSlot(slotKey, id)}
                              >
                                {pName(id, state.players)} ×
                              </span>
                            ))}
              ...
              {pickingTeam && !allDone && (
                ...
                >
                  Picking for <strong>{slotLabel(pickingTeam)}</strong> —{" "}
                  {2 - slots[pickingTeam].length} more player
                  {2 - slots[pickingTeam].length !== 1 ? "s" : ""} needed
                </div>
              )}
                  ...
                  >
                    {filteredUnassigned.map((p) => (
                      <div
                        ...
                        style={{
                          cursor: pickingTeam ? "pointer" : "not-allowed",
                          opacity: pickingTeam ? 1 : 0.5,
                        }}
                        onClick={() => pickingTeam && pickPlayer(p.id)}
                      >
                        <span style={{ fontWeight: 600 }}>{p.name}</span>
                        <span className="xs text-dd">
                          {p.pts || 0}pts · {p.mmr || 1000} MMR
                        </span>
                      </div>
                    ))}
  ...
  const runnerUpNames = bracket?.runnerUp?.map((id) =>
    pName(id, state.players),
  );
  const thirdPlaceNames = bracket?.thirdPlace?.map((id) =>
    pName(id, state.players),
  );
    ...
    <div className="stack page-fade championship-workspace">
      ...
      {status === "complete" && champ && (
        ...
        >
          ...
          {isAdmin && (
            <button
              className="btn btn-p btn-sm mt12"
              onClick={awardChampionship}
            >
              Award to Profiles
            </button>
          )}
...
function PtsChart({ pid, games, players, roleFilter }) {
  ...
  const playerGames = [...games]
    ...
    .filter(
      (g) =>
        !roleFilter || roleFilter === "ALL" || g.roles?.[pid] === roleFilter,
    )
    .sort((a, b) => new Date(a.date) - new Date(b.date));
  ...
  const pathD = data
    .map(
      (d, i) =>
        `${i === 0 ? "M" : "L"}${toX(i).toFixed(1)},${toY(d.pts).toFixed(1)}`,
    )
    .join(" ");
  const fillD =
    pathD +
            ...
            >
              <div
                style={{
                  fontWeight: 700,
                  fontSize: 13,
                  color: hov.won ? "var(--green)" : "var(--red)",
                  marginBottom: 2,
                }}
              >
                {hov.won ? "▲" : "▼"} {hov.pts} pts
              ...
              <div className="text-dd">
                {hov.scoreA}–{hov.scoreB} vs {hov.opps}
              </div>
              <div className="text-dd" style={{ fontSize: 10, marginTop: 2 }}>
                {new Date(hov.date).toLocaleDateString("en-GB", {
                  day: "numeric",
                  month: "short",
                })}
              </div>
...
function StatsView({ state, onSelectPlayer, seasonFilter, setSeasonFilter }) {
  ...
  const scopedGames = (state.games || []).filter((g) =>
    gameInSeason(g, activeSeason),
  );
  const scopedStats = computeWindowPlayerStats(state.players, scopedGames);
  const sorted = [...state.players].sort((a, b) => {
    if (posFilter === "ATK")
      return (b.mmr_atk || b.mmr || 0) - (a.mmr_atk || a.mmr || 0);
    if (posFilter === "DEF")
      return (b.mmr_def || b.mmr || 0) - (a.mmr_def || a.mmr || 0);
  ...
  function getStats(p) {
    ...
    const losses = playerGames.filter((g) => {
      const onA = g.sideA.includes(p.id);
      return (onA && g.winner === "B") || (!onA && g.winner === "A");
    });
    const avgGain = wins.length
      ? Math.round(
          wins.reduce(
            (s, g) => s + (g.perPlayerGains?.[p.id] ?? g.ptsGain),
            0,
          ) / wins.length,
        )
      : 0;
    const avgLoss = losses.length
      ? Math.round(
          losses.reduce(
            (s, g) => s + (g.perPlayerLosses?.[p.id] ?? g.ptsLoss),
            0,
          ) / losses.length,
        )
      : 0;
    const biggestMargin = wins.reduce(
      (best, g) => Math.max(best, Math.abs(g.scoreA - g.scoreB)),
      0,
    );
    const longestStreak = (() => {
      let best = 0,
  ...
  function calcNetPts(pid, pgames) {
    return pgames.reduce((acc, g) => {
      const won =
        (g.sideA.includes(pid) && g.winner === "A") ||
        (g.sideB.includes(pid) && g.winner === "B");
      return (
        acc +
        (won
          ...
          <div className="grid-2" style={{ gap: 0 }}>
            ...
            >
              ...
              <div className="stat-val" style={{ fontSize: 16 }}>
                {activeSeason.startAt &&
                !isNaN(Date.parse(activeSeason.startAt)) ? (
                  new Date(activeSeason.startAt).toLocaleDateString("en-GB", {
                    weekday: "short",
                    month: "short",
                    day: "numeric",
                  })
            ...
            >
              ...
              <div className="stat-val">
                {(() => {
                  ...
                  return scopedGames.filter((g) => new Date(g.date) >= d)
                    .length;
                })()}
              </div>
            </div>
      ...
      <div className="grid-2 stats-columns" style={{ alignItems: "start" }}>
        <div className="card stats-picker">
          <div className="card-header">
            ...
            <select
              ...
              value={seasonFilter}
              onChange={(e) => setSeasonFilter(e.target.value)}
              style={{ fontSize: 11, padding: "4px 8px", maxWidth: 180 }}
            >
              <option value="current">Current season</option>
              <option value="all">All seasons</option>
              {(state.seasons || []).map((se) => (
          ...
          <div style={{ padding: 14 }}>
            <div className="fac" style={{ gap: 4, marginBottom: 8 }}>
              {["ALL", "ATK", "DEF"].map((f) => (
                <button
                  key={f}
                  className={`btn btn-sm ${posFilter === f ? "btn-p" : "btn-g"}`}
        ...
        {selected ? (
          (() => {
            ...
            const pGames = scopedGames.filter(
              (g) =>
                g.sideA.includes(selected.id) || g.sideB.includes(selected.id),
            );
              ...
              <div className="card">
                <div className="card-header">
                  ...
                  >
                    ...
                    <span className="xs text-dd">
                      Rank #{rank} · {st.totalGames} games played
                    </span>
                  </div>
                  <button
                    className="btn btn-g btn-sm"
                    onClick={() => onSelectPlayer(selected)}
                  >
                    Profile
                  </button>
                </div>
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      <PtsChart
                        key={`${selected.id}-${posFilter}`}
                        pid={selected.id}
                        games={scopedGames}
                        players={state.players}
                        roleFilter={posFilter}
                      />
                    </div>
                  </div>
                  {(() => {
                    const atkGames = pGames.filter(
                      (g) => g.roles?.[selected.id] === "ATK",
                    );
                    const defGames = pGames.filter(
                      (g) => g.roles?.[selected.id] === "DEF",
                    );
                    ...
                    const roleAvg = (games, type) => {
                      const relevant = games.filter((g) => {
                        const won =
                          (g.sideA.includes(selected.id) && g.winner === "A") ||
                          (g.sideB.includes(selected.id) && g.winner === "B");
                        return type === "win" ? won : !won;
                      });
                      if (!relevant.length) return null;
                      const key =
                        type === "win" ? "perPlayerGains" : "perPlayerLosses";
                      const fallback = type === "win" ? "ptsGain" : "ptsLoss";
                      return Math.round(
                        relevant.reduce(
                          (s, g) =>
                            s + (g[key]?.[selected.id] ?? g[fallback] ?? 0),
                          0,
                        ) / relevant.length,
                      );
                    };
                          ...
                          {hasRoleData ? (
                            <>
                              ...
                              >
                                ...
                                >
                                  {roleAvg(atkGames, "win") != null && (
                                    <span
                                      className="stat-val am"
                                      style={{ fontSize: 16 }}
                                    >
                                      +{roleAvg(atkGames, "win")}
                                    </span>
                                  )}
                                  {roleAvg(atkGames, "loss") != null && (
                                    <span
                                      className="stat-val"
                                      style={{
                                        fontSize: 16,
                                    ...
                                    >
                                      −{roleAvg(atkGames, "loss")}
                                    </span>
                                  )}
                                  {atkGames.length === 0 && (
                                    <span className="xs text-dd">no games</span>
                                  )}
                                </div>
                              </div>
                              <div
                              ...
                              >
                                ...
                                <div
                                  ...
                                  style={{ gap: 4, marginTop: 2 }}
                                >
                                  {roleAvg(defGames, "win") != null && (
                                    <span
                                      className="stat-val am"
                                      style={{ fontSize: 16 }}
                                    >
                                      +{roleAvg(defGames, "win")}
                                    </span>
                                  )}
                                  {roleAvg(defGames, "loss") != null && (
                                    <span
                                      className="stat-val"
                                      style={{
                                        fontSize: 16,
  ...
  function getBalancings(pids) {
    ...
    return [
      ...
      .map(([t1, t2]) => {
        const mmr1 =
          t1.reduce(
            (s, id) => s + (players.find((p) => p.id === id)?.mmr || 1000),
            0,
          ) / 2;
        const mmr2 =
          t2.reduce(
        ...
        return {
          t1,
          t2,
          mmr1: Math.round(mmr1),
          mmr2: Math.round(mmr2),
  ...
  return (
    <div className="card">
      ...
      <div style={{ padding: 14 }}>
        ...
        >
          Select 4 players to see all possible fair matchups ranked by MMR
          balance.
        </div>
            ...
            {matchups.map(({ t1, t2, mmr1, mmr2, diff, balance }, i) => (
              ...
              >
                ...
                >
                  ...
                  <span className={`tag ${i === 0 ? "tag-w" : "tag-a"}`}>
                    {balance}% balanced
                  </span>
                </div>
                <div
                ...
                >
                  <div>
                    {t1.map((id) => (
                      <div key={id} className="bold" style={{ fontSize: 12 }}>
        ...
        {selected.length > 0 && selected.length < 4 && (
          <div className="msg msg-w" style={{ marginTop: 8 }}>
            Select {4 - selected.length} more player
    ...
    <div className="stack page-fade seasons-workspace">
      {currentSeason && seasonProgress ? (
          ...
          <div>
            ...
            {editingNextDate && isAdmin && (
              ...
              >
                ...
                <div className="fac" style={{ gap: 6 }}>
                  <button className="btn btn-p btn-sm" onClick={saveNextDate}>
                    Save
                  </button>
                  {state.nextSeasonDate && (
                    <button
                      ...
                      onClick={() => {
                        setNextDateInput("");
                        setState((s) => ({ ...s, nextSeasonDate: null }));
                  ...
                  <button
                    className="btn btn-g btn-sm"
                    onClick={() => setEditingNextDate(false)}
                  >
                    Cancel
                  </button>
                </div>
            ...
            {seasonProgress.hasEnd ? (
              ...
              >
                ...
                <div>
                  ...
                  <div className="xs text-dd">
                    No end date set{isAdmin ? " — set one above" : ""}
                  </div>
                </div>
              </div>
            )}
        ...
        >
          ...
          >
            Start a season to begin tracking rankings and progress.
          </div>
          {isAdmin && (
            <button
              className="btn btn-p"
      ...
      {closedSeasons.length === 0 ? (
        ...
        [...closedSeasons].reverse().map((season, idx) => {
          ...
          const seasonGames = (state.games || []).filter((g) =>
            gameInSeason(g, season),
          );
          const seasonStats = computeWindowPlayerStats(
            state.players,
            seasonGames,
          );
          const ranked = [...(state.players || [])].sort(
            (a, b) =>
              (seasonStats[b.id]?.pts || 0) - (seasonStats[a.id]?.pts || 0),
          );
          const topThree = ranked
            .slice(0, 3)
            .filter(
              (p) =>
                seasonStats[p.id]?.wins > 0 || seasonStats[p.id]?.losses > 0,
            );
          const startDate =
            season.startAt && !isNaN(Date.parse(season.startAt))
              ? new Date(season.startAt).toLocaleDateString("en-GB", {
                  day: "numeric",
                  month: "short",
                  year: "numeric",
                })
              : "?";
          const endDate =
            season.endAt && !isNaN(Date.parse(season.endAt))
              ? new Date(season.endAt).toLocaleDateString("en-GB", {
                  day: "numeric",
                  month: "short",
                  year: "numeric",
                })
      ...
      <div className="stack">
        <div className="card">
          <div className="card-header">
            ...
            <div className="fac">
              <button
                className="btn btn-g"
                onClick={() => {
                  setDraft(state.rules || DEFAULT_RULES);
                  setEditing(false);
                }}
              >
                Cancel
          ...
          <div style={{ padding: 18 }}>
            <div className="msg msg-w sm mb12">
              Supports Obsidian markdown: # headings, **bold**, - lists, `code`,
              ---
            </div>
            <textarea
              className="inp"
...
function AdvancedPanel({
  ...
  const lastWriteAt = state?._meta?.lastWriteAt
    ? new Date(state._meta.lastWriteAt).toLocaleString("en-GB")
    : "—";
  ...
  async function restoreState(row) {
    if (!row) return;
    setLoading(true);
    const { data, error } = await supabase.rpc("replace_state_admin", {
      new_state: row.state,
      p_action: "restore_state",
    });
    if (error) showToast("Restore failed", "err");
    else {
      setState(normaliseState(data || row.state));
  ...
  async function hardReset() {
    ...
    if (!error) {
      setState(normaliseState(data || next));
      showToast("Leaderboard reset", "ok");
    } else showToast("Reset failed", "err");
  }
  function resetSeasonPoints() {
    const ok = confirm(
      "Start a new season? Points, MMR, and streaks reset. History and stats stay.",
    );
    if (!ok) return;
  ...
  function publishAnnouncement() {
    if (!annBody.trim()) {
      showToast("Announcement body required", "err");
      return;
    }
    const now = new Date();
      ...
      <div style={{ padding: 16 }}>
        ...
        >
          ...
          <label className="fac xs text-d" style={{ gap: 6, padding: "0 6px" }}>
            <input
              type="checkbox"
              checked={autoSeasonAnnouncement}
              onChange={(e) => setAutoSeasonAnnouncement(e.target.checked)}
            />
          ...
          <div style={{ padding: 14 }}>
            ...
            {selected?.state?._meta && (
              <div className="xs text-dd" style={{ marginTop: 8 }}>
                Backup writer: {selected.state._meta.lastWriterId || "—"} ·{" "}
                {selected.state._meta.lastWriteAt
                  ? new Date(selected.state._meta.lastWriteAt).toLocaleString(
                      "en-GB",
                    )
                  : "—"}
              </div>
        ...
        >
          <div className="card-header">
            ...
            <div className="fac" style={{ gap: 6 }}>
              {state.announcement && (
                <span
                  className={`tag ${isAnnouncementActive(state.announcement) ? "tag-w" : new Date(state.announcement.startsAt) > new Date() ? "tag-b" : "tag-a"}`}
                >
                  {isAnnouncementActive(state.announcement)
                    ? "Active"
                    : new Date(state.announcement.startsAt) > new Date()
                      ? "Scheduled"
                      : "Expired"}
                </span>
              )}
              <button
                className={`btn btn-sm ${annPreview ? "btn-p" : "btn-g"}`}
                onClick={() => setAnnPreview((v) => !v)}
              >
                ...
                <div className="grid-2">
                  <div>
                    ...
                    <input
                      className="inp"
                      type="datetime-local"
                      value={annScheduleStart}
                      onChange={(e) => setAnnScheduleStart(e.target.value)}
                    />
                  </div>
                  <div>
                    ...
                    <input
                      className="inp"
                      type="datetime-local"
                      value={annScheduleEnd}
                      onChange={(e) => setAnnScheduleEnd(e.target.value)}
                    />
                  </div>
                </div>
              </div>
            ...
            >
              <div>
                {state.announcement && (
                  <div
                    ...
                    style={{ display: "flex", alignItems: "center", gap: 6 }}
                  >
                    {new Date(state.announcement.startsAt).toLocaleString(
                      "en-GB",
                      {
                        day: "numeric",
                        month: "short",
                    ...
                    {" → "}
                    {new Date(state.announcement.endsAt).toLocaleString(
                      "en-GB",
                      {
                        day: "numeric",
                        month: "short",
              ...
              <div className="fac" style={{ gap: 8 }}>
                <button className="btn btn-p" onClick={publishAnnouncement}>
                  {annScheduleStart && new Date(annScheduleStart) > new Date()
                    ? "Schedule"
                    : "Publish"}
                </button>
                {state.announcement && (
        ...
        >
          ...
          <div style={{ padding: 14, display: "grid", gap: 10 }}>
            <div className="field">
              ...
              <select
                className="inp"
                value={exportSeasonFilter}
                onChange={(e) => setExportSeasonFilter(e.target.value)}
              >
                <option value="current">Current season</option>
                <option value="all">All seasons</option>
                {(state.seasons || []).map((s) => (
            ...
            <div className="fac" style={{ gap: 8, flexWrap: "wrap" }}>
              ...
              <button
                className="btn btn-g"
                onClick={() => exportPlayersCsv(state, exportSeasonFilter)}
              >
                Export Players (CSV)
              </button>
              <button
                className="btn btn-g"
                onClick={() => exportGamesCsv(state, exportSeasonFilter)}
              >
                Export Games (CSV)
              </button>
            </div>
        ...
        >
          ...
          <div style={{ padding: 14, display: "grid", gap: 6 }}>
            ...
            <div className="xs text-dd">
              Last backup (local): {lastBackupLabel}
            </div>
      ...
      <div style={{ padding: 14 }}>
        ...
        >
          ...
          {log.map((e, i) => (
            <div
              key={i}
              style={{ color: colMap[e.type] || "var(--dim)", lineHeight: 1.8 }}
            >
              <span style={{ color: "var(--dimmer)", marginRight: 8 }}>
                {e.time}
              </span>
              {e.msg}
            </div>
          ...
          {running && (
            <div
              style={{
                color: "var(--dimmer)",
                animation: "savingBar 1s infinite alternate",
              }}
            >
              ⋯ running
            </div>
  ...
  return (
    <div className="login-wrap">
      <div className="login-box">
        ...
        <button className="btn btn-p w-full mt16" onClick={go} disabled={busy}>
          {busy ? "Checking…" : "Login"}
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  useEffect(() => {
    async function initState() {
      try {
        ...
        const missingCurrentKey =
          currentKey !== "all" && !keys.includes(currentKey);
        if (
          (hasOldMonthKeys || missingCurrentKey) &&
          loaded.games?.length > 0
        ) {
          const recomputed = computePlacements(loaded.games, loaded.seasons);
          loaded.monthlyPlacements = {
            ...loaded.monthlyPlacements,
            ...recomputed,
          };
          console.log(
            "[placement-migration] recomputed keys:",
  ...
  function setSyncFor(status, ms = 2500) {
    ...
    if (ms)
      syncStatusTimer.current = setTimeout(() => setSyncStatus("idle"), ms);
  }
      ...
      (remoteState) => {
        ...
        if (showToastRef.current)
          showToastRef.current(
            "Sync conflict — remote state applied",
            "warning",
          );
      },
      (newV, meta) => {
  ...
  function handleRemotePayload(payload) {
    ...
    console.log("Applying remote _v" + incomingV);
    isRemoteUpdate.current = true;
    setState(incoming);
  }
        ...
        if (status === "SUBSCRIBED") {
          ...
          if (wasDisconnected.current) {
            ...
            try {
              ...
              console.warn("[sync] catch-up failed:", e);
            }
          }
        }
        if (
  ...
  useEffect(() => {
    const id = setInterval(() => setAnnTick((t) => t + 1), 30000);
    return () => clearInterval(id);
  }, []);
  const activeAnnouncement = isAnnouncementActive(state.announcement)
    ? state.announcement
    : null;
  useEffect(() => {
  ...
  function dismissAnnouncement() {
    ...
    writeLocalNumber(
      `${ANN_DISMISS_PREFIX}${activeAnnouncement.id}`,
      Date.now(),
    );
    setShowAnnouncement(false);
  }
  ...
  function startNewSeason({
    ...
    if (prevSeasons.length && !prevSeasons[prevSeasons.length - 1].endAt)
      prevSeasons[prevSeasons.length - 1] = {
        ...prevSeasons[prevSeasons.length - 1],
        endAt: seasonStart,
      };
    const nextSeason = {
      ...
      <div className="app">
        ...
        {currentSelPlayer && !editPlayer && (
          <PlayerProfile
            ...
            selectedSeasonId={
              profileSeasonId || getCurrentSeason(state)?.id || ""
            }
