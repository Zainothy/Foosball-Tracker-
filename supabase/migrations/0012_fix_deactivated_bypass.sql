-- ensure_player_profile() is SECURITY DEFINER and its lookup has no active
-- filter, so it returned an existing deactivated row unconditionally. The
-- caller (authClient.js) only reaches this RPC when the direct client-side
-- select finds nothing -- which, given the "active = true" self-select RLS
-- policy, is exactly what happens for a deactivated account (RLS hides the
-- row rather than returning it inactive). Net effect: a deactivated account
-- could sign back in via this path instead of being blocked.
...
begin
  ...
  if result.user_id is not null then
    if not result.active then raise exception 'This login has been deactivated'; end if;
    return result;
  end if;
